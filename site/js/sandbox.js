...
const ALL_NAV = [
  ...
  { route: "plans", label: "플랜", navKind: "plans_bug" },
  { routes: ["login", "signup"], label: "계정", navKind: "account" },
  { route: "profile", label: "내 정보", navKind: "target" }
];
...
class SandboxApp {
  ...
  /**
   * 미션 sandbox.entryRoute 기준 초기 화면. enrich 미적용 시에도 allowedRoutes 안에서만 선택.
   * post 단독 진입은 activePostId 없이 깨지므로 board로 내린다.
   */
  static resolveInitialRoute(mission) {
    const m = mission?.sandbox || {};
    const allowed = Array.isArray(m.allowedRoutes) && m.allowedRoutes.length ? m.allowedRoutes : null;
    const preference = ["landing", "contact", "apply", "plans", "signup", "login", "board", "post", "profile"];
    let route = "landing";
    if (m.entryRoute && allowed && allowed.includes(m.entryRoute)) {
      route = m.entryRoute;
    } else if (allowed) {
    ...
    if (route === "post") route = allowed && allowed.includes("board") ? "board" : route;
    return route;
  }
  ...
  headerProfile() {
    const hp = this.mission.sandbox?.headerProfile;
    if (hp === "guest_nav" || hp === "auth_nav" || hp === "default") return hp;
    return "default";
  }

  /**
   * 상단 네비 노출: allowedRoutes + (게시판·프로필은 로그인 후) + 이미 로그인이면 로그인·가입 항목 숨김.
   * sandbox.headerProfile 값은 헤더 DOM `data-header-profile`로 노출된다.
   */
  isNavItemVisible(item) {
    const loggedIn = !!this.state.currentUser;

    if (item.navKind === "account") {
      const canLogin = this.allowed("login") && !loggedIn;
      const canSignup = this.allowed("signup") && !loggedIn;
      return canLogin || canSignup;
    }

    const route = item.route;
    if (!this.allowed(route)) return false;

    if (route === "board" || route === "profile") {
      if (!loggedIn) return false;
    }

    if (loggedIn && (route === "login" || route === "signup")) {
      return false;
    }
  ...
  /** 랜딩 상단 큰 배너(메인 카피·버튼 줄) CTA: 랜딩에 있을 때만 표시 */
  shouldShowHeroStrip() {
    return this.state.route === "landing";
  }
  ...
  buildInitialState() {
    ...
    const initialRoute = SandboxApp.resolveInitialRoute(this.mission);
    st.route = initialRoute;
    if (!st.visited.includes(initialRoute)) {
      st.visited.push(initialRoute);
    }
    if (initialRoute === "board" && m.bugBoardEmptyList) {
      const visible = st.posts.filter((p) => !st.deletedPostIds.has(p.id));
      if (visible.length > 0) {
        st.boardEmptyWhileHasPostsBugObserved = true;
      }
    }
    return st;
  }
  ...
  heroClick(kind) {
    if (kind === "apply" && this.mission.sandbox?.bugHeroApplyToPlans) {
      ...
      this.goRoute("plans", "랜딩배너:apply→플랜(버그)");
      return;
    }
    if (kind === "plans" && this.mission.sandbox?.bugHeroPlansToContact) {
      ...
      this.goRoute("contact", "랜딩배너:plans→문의(버그)");
      return;
    }
    const map = {
      apply: "apply",
  ...
  render() {
    ...
    const navHtml = ALL_NAV.map((item) => {
      if (item.navKind === "account") {
        if (!this.isNavItemVisible(item)) return "";
        const visLogin = this.allowed("login") && !state.currentUser;
        const visSignup = this.allowed("signup") && !state.currentUser;
        const parts = [];
        if (visLogin) {
          const cls = state.route === "login" ? "active" : "";
          parts.push(
            `<button type="button" class="seo-nav-btn ${cls}" data-go="login">로그인</button>`
          );
        }
        if (visSignup) {
          const cls = state.route === "signup" ? "active" : "";
          parts.push(
            `<button type="button" class="seo-nav-btn ${cls}" data-go="signup">회원가입</button>`
          );
        }
        const sep = parts.length > 1 ? '<span class="seo-nav-account-sep" aria-hidden="true">·</span>' : "";
        return `<span class="seo-nav-account" role="group" aria-label="계정">${parts.join(sep)}</span>`;
      }
      const visible = this.isNavItemVisible(item);
      if (item.route === "post") return "";
      const active = state.route === item.route || (item.route === "board" && state.route === "post");
      const cls = active ? "active" : "";
      const dis = visible ? "" : "disabled";
      if (item.navKind === "plans_bug") {
        return `<button type="button" class="seo-nav-btn ${cls} ${dis}" data-nav="plans-bug" ${visible ? "" : "disabled"}>${item.label}</button>`;
      }
      return `<button type="button" class="seo-nav-btn ${cls} ${dis}" data-go="${item.route}" ${visible ? "" : "disabled"}>${item.label}</button>`;
    }).join("");
    ...
    this.container.innerHTML = `
      <div class="seo-demo">
        <header class="seo-top" data-header-profile="${escapeHtml(this.headerProfile())}">
          <div class="seo-logo">Studio <span>One</span></div>
          <nav class="seo-nav" aria-label="주 메뉴">${navHtml}</nav>
          <div class="seo-session">${userBar}</div>
        </header>
        ${
          this.shouldShowHeroStrip()
            ? `<section class="seo-hero">
          <h1>팀과 고객을 잇는 한 곳</h1>
          <p>문의·신청·플랜까지 한 화면에서 다루는 제품 데모입니다. 이 미션에서 허용된 메뉴만 활성화됩니다.</p>
          <p class="seo-hero-caption muted small">아래 버튼 줄은 홈(랜딩) 맨 위의 <strong>큰 안내 영역</strong>입니다. 실무에서는 종종 <strong>히어로 섹션</strong>이라고 부릅니다. 상단 메뉴와 <strong>다른 진입 경로</strong>이므로, 라벨과 실제 이동 화면이 맞는지 비교해 보세요.</p>
          <div class="seo-cta-row">
            ...
            <button type="button" class="seo-cta" data-hero="plans">플랜 보기</button>
        ...
        <div class="seo-body">
          <section class="seo-view ${state.route === "landing" ? "active" : ""}" data-view="landing">
            <h2>서비스 소개</h2>
            <p class="desc">문의와 신청, 플랜 비교로 시작할 수 있습니다. 상단 메뉴와 아래 버튼으로 이동해 보세요.</p>
          ...
          <section class="seo-view ${state.route === "login" ? "active" : ""}" data-view="login">
            <h2>로그인</h2>
            <p class="desc">가입 시 사용한 이메일과 비밀번호로 로그인합니다. (연습용 시드 계정은 미션 안내를 따르세요.)</p>
